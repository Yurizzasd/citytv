...
export default function Watch() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const best = await playerService.getWithFallback(
          anilistId,
          epNum,
          audio,
          ordered.length ? ordered : PROVIDER_PRIORITY,
          { title: anime?.title },
        );
        if (!alive) return;
        if (!best.fallback) {
          ...
          setSearch({ provider: 'sugoi', audio }, { replace: true });
        }
        setSource(best);
  ...
  }, [anilistId, epNum, audio, provider, anime?.title]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div className="watch-layout">
          <div>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
              ...
              {source?.fallback && <span className="chip">Fonte reserva (Sugoi)</span>}
            </div>
